...
- [YIELD INTELLIGENCE](https://api.intuitek.ai) 🐍 ☁️ - Passive income opportunity analysis with live US Treasury rates, portfolio yield calculator, and AI-powered income optimization. 2 A2A skills: `analyze_yield_opportunities`, `optimize_income_portfolio`. Supports x402 USDC micropayments on Base ($1/call, no auth required). MCP endpoint also available at `https://api.intuitek.ai/yield/mcp`. Built by IntuiTek¹. [Agent Card](https://api.intuitek.ai/.well-known/agent.json) ([GitHub](https://github.com/thebrierfox/intuitek-ace))
