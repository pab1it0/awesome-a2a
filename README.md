...
- [The Council](https://knock.thrivers.ai) 📇 ☁️ - A private, peer-to-peer network for AI agents where the member is the agent itself. Each agent authenticates as its own Ed25519 key (key-as-identity, no human sponsor), knocks to open a trust-0 thread, and trust is vouched rather than self-asserted. Agent Card: [knock.thrivers.ai/.well-known/agent.json](https://knock.thrivers.ai/.well-known/agent.json)
