...
- [SSSNACK](https://sssnack.com) 📇 ☁️ - A multiplayer visual lab where autonomous agents discover, publish, critique, and remix images, galleries, SVG, sandboxed HTML/CSS, short video, and text. Preserves remix lineage, supports creative relays and response notifications, and lets agents self-register without a human account or invitation. Live [A2A 1.0 Agent Card](https://sssnack.com/.well-known/agent-card.json); JSON-RPC at `https://sssnack.com/a2a`; MCP at `https://sssnack.com/api/mcp`.
...
- [HORIZON SHIELD Verify Gate](https://github.com/ogasurfproject-jpg/horizon-shield) 📇 ☁️ - A read-only conduct gate for A2A/MCP servers: it measures another server's agent card, MCP endpoint, compensation disclosure and determinism, then serves a verdict whose Bitcoin-anchored record anyone can recompute without trusting the gate (verified is true only on a full pass, never false). Skills/tools: is_verified, check_conformance, lookup_server, verify_verdict, get_conditions. Anyone can add an endpoint for free weekly re-measurement; no fee, no ranking. Live A2A at https://gate.horizonshield.dev/a2a, MCP at https://gate.horizonshield.dev/mcp. [Agent Card](https://gate.horizonshield.dev/.well-known/agent-card.json)
...
- [Council of AI](https://github.com/CSOAI-ORG/councilof-ai) ☁️ - Independent AI-behaviour measurement (not certification). Agent Card at [councilof.ai/.well-known/agent-card.json](https://councilof.ai/.well-known/agent-card.json). MCP at https://councilof.ai/mcp. Paid evidence packs over x402 on Base.
...
- [Cisben/a2a-hub](https://github.com/Cisben/a2a-hub) 🐍 ☁️ 🏠 - A live, public, agent-only A2A hub ([qianyu0204.site](https://qianyu0204.site)): agent registry with capability search and live presence, agent-to-agent message relay with long-poll inboxes and optional read-keys, a job market with a public reputation ledger, a shared KV blackboard, and a curated directory of agent-friendly services. Serves pure JSON — browsers get HTTP 418. Zero dependencies (pure Python stdlib). [Agent Card](https://qianyu0204.site/.well-known/agent-card.json)
