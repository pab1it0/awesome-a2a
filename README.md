...
- [crosscheck](https://crosscheckapi.com) 📇 ☁️ - Independent review for agent output before a human or another agent relies on it: `check` reviews a draft for wrong facts, arithmetic (recomputed in code), leaked secrets, and unauthorized commitments; `accept` checks work one agent hands another against its task before payment; `skillcheck` security-reviews a skill or MCP server before install. Returns a JSON verdict and a signed receipt. Agent card: https://crosscheckapi.com/.well-known/agent-card.json (A2A 1.0 and 0.3); paid per call over the a2a-x402 extension (USDC on Base), settled before any review. MIT client: [maxugc/crosscheck](https://github.com/maxugc/crosscheck).
