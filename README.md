...
- [402cron](https://402cron.com) 📇 ☁️ - Paid cron for AI agents. Its A2A 1.0 endpoint (JSON-RPC at `402cron.com/a2a`) is a read-only receptionist: every message gets a guide with live prices, buy URLs and docs. Scheduling itself (signed HTTP deliveries to your endpoint on a cron schedule, with retries) runs over REST or MCP after an x402 payment in USDC on Base; no account. Agent card at `/.well-known/agent-card.json`.
